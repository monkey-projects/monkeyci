...
(defn read-events
  ...
  [org-id token on-recv-evt]
  (if org-id
    (do
      (log/debug "Starting reading events for org" org-id)
      ...
      (let [src (js/EventSource. (str events-url "/org/" org-id "/events?authorization=" token)
                                 (clj->js {}))]
        (set! (.-onmessage src)
              (fn [evt]
