(ns monkey.ci.metrics.entity-stats-test
  (:require [clojure.test :refer [deftest testing is]]
            [monkey.ci.metrics.entity-stats :as sut]
            [monkey.metrics.prometheus :as p]
            [monkey.ci.storage :as st]
            [monkey.ci.test.storage :as ts]))

(defn- gauge-val [g]
  (.. g
      (collect)
      (getDataPoints)
      (get 0)
      (getValue)))

(deftest user-count-gauge
  (ts/with-memory-store st
    (testing "provides number of users in storage"
      (let [reg (p/make-registry)
            g (sut/user-count-gauge st reg)]
        (is (= 0.0 (gauge-val g)))
        (is (some? (st/save-user st (ts/gen-user))))
        (is (= 1.0 (gauge-val g)))))))

(deftest org-count-gauge
  (ts/with-memory-store st
    (testing "provides number of orgs in storage"
      (let [reg (p/make-registry)
            g (sut/org-count-gauge st reg)]
        (is (= 0.0 (gauge-val g)))
        (is (some? (st/save-org st (ts/gen-org))))
        (is (= 1.0 (gauge-val g)))))))

(deftest repo-count-gauge
  (ts/with-memory-store st
    (testing "provides number of repos in storage"
      (let [reg (p/make-registry)
            g (sut/repo-count-gauge st reg)]
        (is (= 0.0 (gauge-val g)))
        (is (some? (st/save-repo st (ts/gen-repo))))
        (is (= 1.0 (gauge-val g)))))))

(deftest email-reg-count-gauge
  (ts/with-memory-store st
    (testing "provides number of email registrations in storage"
      (let [reg (p/make-registry)
            g (sut/email-reg-count-gauge st reg)]
        (is (= 0.0 (gauge-val g)))
        (is (some? (st/save-email-registration st (ts/gen-email-registration))))
        (is (= 1.0 (gauge-val g)))))))
