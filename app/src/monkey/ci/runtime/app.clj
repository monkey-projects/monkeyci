...
;;    :runtime    (co/using
;;                 (new-runtime config)
;;                 [:mailman :artifacts :cache :workspace :logging :git :build :api-config :build-cache])
...
;;    :local/routes (co/using
;;                   (new-local-routes config)
;;                   [:local/mailman])))
